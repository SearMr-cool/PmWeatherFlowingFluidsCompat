...
@EventBusSubscriber(
        modid = PmWeatherFlowingFluidsCompatServer.MODID
)
public class Config {
    ...
    private static final ModConfigSpec.BooleanValue storm_surge_biome_check;
    public static boolean stormSurgeBiomeCheck;
    ...
    private static void onLoad(ModConfigEvent event) {
        if (!(event instanceof ModConfigEvent.Unloading) && event.getConfig().getSpec() == thing){
            ...
            stormSurgeBiomeCheck = (Boolean) storm_surge_biome_check.get();
    ...
    static  {
        ...
        storm_surge_biome_check = BUILDER.comment("Enable if you wish to make storm surge chunks require certain biome types (recommended to enable with mods that consistently place beach biomes beside oceans eg. Lithosphere)").define("stormsurgebiomecheck",false);
